...
export default function WaterLogModal({ isOpen, onClose, onLog }: WaterLogModalProps) {
  ...
  const y = useMotionValue(valueToY(250));

  function valueToY(val: number) {
    return (1 - val / max) * trackHeight;
  }
              ...
              {[250, 500, 1000, 2000].map((amt) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      setValue(amt);
                      animate(y, valueToY(amt), { type: "tween", ease: "easeInOut", duration: 0.5 });
                    }}
              ...
              <AnimatePresence mode="wait">
                {!isConfirming ? (
                  <motion.button
                    ...
                    className="px-6 py-2 rounded-lg bg-blue-500 text-white shadow-md"
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -10 }}
                    transition={{ duration: 0.3 }}
                  >
                    Log {formatLabel(value)}
                  </motion.button>
                ) : (
                  <motion.div
                    key="circle"
